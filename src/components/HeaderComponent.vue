  ...
  <v-app-bar app>
    <v-container>
      <v-row align="center" class="flex-nowrap">
        ...
        <v-col class="text-right d-flex right_btns">
      ...
      if (this.isLogin) {
        try {
          const response = await axios.get(
            `${process.env.VUE_APP_API_BASE_URL}/api/notifications/list`,
            {
              headers: {
    ...
    async markAsReadAndNavigate(notification, index) {
      if (notification.delYN === "N") {
        try {
          ...
          await axios.get(
            `${process.env.VUE_APP_API_BASE_URL}/api/notifications/update/${id}`,
            null,
            {
              headers: {
                Authorization: `Bearer ${localStorage.getItem("token")}`,
    ...
    navigate(section) {
      if (section === "강좌") {
        ...
        this.$router.push({
          ...
          params: { category: "subjectRecommend" },
        });
      } else if (section === "이벤트") {
